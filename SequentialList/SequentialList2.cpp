    ...
    void searchWord() {
        ...
        while (true){
            ...
            while(l + 1 != r) {
                asl++;
                int mid = (l + r) / 2;
                int cmp = strcmp(words[mid].word, word);
                if(cmp > 0) {
                    r = mid;
                }else{
                    l = mid;
            ...
            if(l >= 0 && strcmp(words[l].word, word) == 0) {
                auto end = chrono::high_resolution_clock::now();
                totalAsl += asl;
                totalSearches++;
                cout << words[l].word << " 出现的次数为: " << words[l].frequency << endl
                         << "查找次数: " << asl << endl
...
int main()
{
    ...
    const char* outFilename = "../Output/OutFile3.txt";
